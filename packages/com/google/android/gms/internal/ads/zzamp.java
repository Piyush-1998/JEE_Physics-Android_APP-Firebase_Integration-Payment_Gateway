...
public final class zzamp {
    ...
    public zzamp(JSONObject jSONObject) throws JSONException {
        ...
        for (int i2 = 0; i2 < jSONArray.length(); i2++) {
            try {
                zzamq zzamqVar = new zzamq(jSONArray.getJSONObject(i2));
                boolean z = true;
                if ("banner".equalsIgnoreCase(zzamqVar.zzdkg)) {
                    this.zzdjk = true;
                }
                arrayList.add(zzamqVar);
                if (i < 0) {
                    Iterator<String> it = zzamqVar.zzdjn.iterator();
                    while (true) {
                        if (it.hasNext()) {
        ...
        this.zzdjd = jSONObject.optLong("timeout_ms", -1L);
        JSONObject optJSONObject = jSONObject.optJSONObject("settings");
        if (optJSONObject != null) {
            this.zzdir = optJSONObject.optLong("ad_network_timeout_millis", -1L);
            ...
            this.zzdiz = optLong > 0 ? 1000 * optLong : -1L;
