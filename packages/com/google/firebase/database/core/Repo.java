...
public class Repo implements PersistentConnection.Delegate {
    ...
    private void sendTransactionQueue(final List<TransactionData> list, final Path path) {
        ...
        for (TransactionData transactionData2 : list) {
            transactionData2.status = TransactionStatus.SENT;
            TransactionData.access$2008(transactionData2);
            latestState = latestState.updateChild(Path.getRelative(path, transactionData2.path), transactionData2.currentOutputSnapshotRaw);
        }
            ...
            public void onRequestResult(String str, String str2) {
                ...
                if (fromErrorCode.getCode() == -1) {
                    for (TransactionData transactionData4 : list) {
                        if (transactionData4.status == TransactionStatus.SENT_NEEDS_ABORT) {
                            transactionData4.status = TransactionStatus.NEEDS_ABORT;
                        } else {
                            transactionData4.status = TransactionStatus.RUN;
                        }
                    }
                } else {
                    for (TransactionData transactionData5 : list) {
                        transactionData5.status = TransactionStatus.NEEDS_ABORT;
                        transactionData5.abortReason = fromErrorCode;
                    }
                }
